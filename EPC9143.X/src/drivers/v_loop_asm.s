...
; Compute compensation filter term
	...
	mac w4*w6, a, [w8]+=2, w4, [w10]+=2, w6    ; multiply control output (n-3) from the delay line with coefficient A3
...
; Compute B-Term of the compensation filter
	...
	mac w4*w6, b, [w8]+=2, w4, [w10]+=2, w6    ; multiply & accumulate error input (n-3) from the delay line with coefficient B3 and prefetch next operands
