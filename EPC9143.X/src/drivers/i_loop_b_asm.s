...
;  Author:      M91406
;  Date/Time:   02/08/2020 3:05:17 AM
; **********************************************************************************
;  2P2Z Control Library File (Single Bitshift-Scaling Mode)
; **********************************************************************************
...
;------------------------------------------------------------------------------
	...
	.global _i_loop_b_Update
_i_loop_b_Update:    ; provide global scope to routine
...
; Enable/Disable bypass branch target with dummy read of source buffer
	goto I_LOOP_B_EXIT_LOOP    ; when enabled, step over dummy read and go straight to EXIT
	I_LOOP_B_BYPASS_LOOP:    ; Enable/Disable bypass branch target to perform dummy read of source to clear the source buffer
	mov [w0 + #offSourceRegister], w2    ; load pointer to input source register
	mov [w2], w1    ; move value from input source into working register
	mov [w0 + #offPtrControlInput], w2    ; load pointer address of target buffer of most recent controller input from data structure
	mov w1, [w2]    ; copy most recent controller input value to given data buffer target
	I_LOOP_B_EXIT_LOOP:    ; Exit control loop branch target
